...
&nbsp;
## **GenServer**
In the previous lesson, we used agents to represent our buckets. In the introduction to mix, we specified we would like to name each bucket so we can do the following:
...
Since agents are processes, each bucket has a process identifier (pid), but buckets do not have a name. Back in the lesson about processes, we have learned that we can register processes in Elixir by giving them atom names:

    iex> Agent.start_link(fn -> %{} end, name: :shopping)
    {:ok, #PID<0.43.0>}
    iex> ValueStorage.Bucket.put(:shopping, "milk", 1)
    :ok
    iex> ValueStorage.Bucket.get(:shopping, "milk")
    1

However, naming dynamic processes with atoms is a bad idea. If we use atoms, we would need to convert the bucket name to atoms, and we should never convert user input to atoms. This is because atoms are not garbage collected. Once an atom is created, it is never reclaimed. In practice, it is likely app reaches the Erlang VM limit for the maximum number of atoms before system runs out of memory, which will bring your system down regardless.

Instead, we will create our own process registry that associates the bucket name to the bucket process.
...
We will use a **GenServer** to create a registry process that can monitor the bucket processes.
...
In a **GenServer**, the code above would be two separate functions:
...
There is quite a bit more ceremony in the **GenServer** code but it brings some benefits too.
...
Create a new file at lib/registry.ex with the following contents:
...
In order to invoke the callbacks above, we need to go through the corresponding GenServer functions. Let's start a registry, create a named bucket, and then look it up:
    ...
    $ iex -S mix
    iex> {:ok, registry} = GenServer.start_link(ValueStorage.Registry, :ok)
    {:ok, #PID<0.136.0>}
...
Edit the file at lib/registry.ex, filling in the blanks for the client API:
...
There are other tuple formats both handle_call/3 and handle_cast/2 callbacks may return. There are also other callbacks like terminate/2 and code_change/3 that we could implement. Explore the full [GenServer documentation](https://hexdocs.pm/elixir/GenServer.html) to learn more about those.


&nbsp;
### **Testing a GenServer**

Testing a GenServer is not much different from testing an agent. We will spawn the server on a setup callback and use it throughout our tests. Create a file at test/registry_test.exs with the following:
...
There is one important difference between the setup block we wrote for ValueStorage.Registry and the one we wrote for ValueStorage.Bucket. Instead of starting the registry by hand by calling ValueStorage.Registry.start_link/1, we instead called the start_supervised!/2 function, passing the ValueStorage.Registry module.

The start_supervised! function was injected into our test module by use ExUnit.Case. It does the job of starting the ValueStorage.Registry process, by calling its start_link/1 function. The advantage of using start_supervised! is that ExUnit will guarantee that the registry process will be shutdown before the next test starts. It helps guarantee that the state of one test is not going to interfere with the next one in case they depend on shared resources.

When starting processes during your tests, the use of start_supervised! should be preferred. It is recommended to change the setup block in bucket_test.exs to use start_supervised! too.
...
Finally, different from the other callbacks, we have defined a *catch-all* clause for handle_info/2 that discards any unknown message. This mechanism is explained in next section.
...
To help developers remember the differences between call, cast and info, the supported return values and more, check the [tiny GenServer cheat sheet](https://elixir-lang.org/cheatsheets/gen-server.pdf).


&nbsp;
### **Monitors or links?**

We have previously learned about links in the lesson about processes. Now, with the registry complete, you may be wondering: when should we use monitors and when should we use links?

Links are bi-directional. If you link two processes and one of them crashes, the other side will crash too (unless it is trapping exits). A monitor is uni-directional: only the monitoring process will receive notifications about the monitored one. In other words: use links when you want linked crashes, and monitors when you just want to be informed of crashes and exits.
